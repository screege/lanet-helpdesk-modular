...
class MonitoringModule:
    ...
    def __init__(self, config_manager, database_manager):
        ...
        # Initialize BitLocker collector if available
        ...
        if BITLOCKER_AVAILABLE:
            try:
                ...
                self.logger.warning(f"Failed to initialize BitLocker collector: {e}")
        ...
        try:
            ...
            if isinstance(disk_usage, (int, float)) and disk_usage > self.disk_threshold:
                issues_found.append({
                    ...
                    'subject': f'Disco principal casi lleno ({disk_usage}%)',
                    'description': f'El disco principal está {disk_usage}% lleno. Se recomienda liberar espacio urgentemente.',
                    'value': disk_usage
                })
    ...
    def get_bitlocker_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if bitlocker_info.get('supported'):
                ...
                self.logger.info(f"BitLocker collection completed: {protected_count}/{volume_count} volumes protected")
            else:
            ...
            self.logger.error(f"Error collecting BitLocker information: {e}")
            return {
                'supported': False,
