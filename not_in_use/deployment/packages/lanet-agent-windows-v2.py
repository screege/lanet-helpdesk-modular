...
class LANETAgentV2:
    def __init__(self):
        self.version = "2.0.0"
        self.config_file = Path("C:/temp/lanet_agent/config.ini")
        self.log_file = Path("C:/temp/lanet_agent/agent.log")
        self.setup_logging()
        self.load_config()
    ...
    def register_with_token(self, installation_token):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.info(f"Registration response received: {response_data.get('message', 'No message')}")

                # El backend devuelve los datos dentro de 'data'
                if 'data' in response_data and response_data['data']:
                    registration_data = response_data['data']
                    self.logger.info("Using registration data from 'data' field")
                else:
                    registration_data = response_data
                    self.logger.info("Using registration data from root level")
                ...
                missing_fields = [field for field in required_fields if field not in registration_data]

                if missing_fields:
                    self.logger.error(f"Missing required fields in registration response: {missing_fields}")
                    return False
                ...
                if 'config' in registration_data:
                    ...
                    self.config['INTERVALS']['heartbeat'] = str(config.get('heartbeat_interval', 60))
                    self.config['INTERVALS']['inventory'] = str(config.get('inventory_interval', 3600))
                    self.config['INTERVALS']['metrics'] = str(config.get('metrics_interval', 300))
