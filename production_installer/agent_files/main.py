...
def main():
    ...
    try:
        ...
        if not agent.is_registered():
            # Check if we're in service mode and have an installation token
            service_mode = os.environ.get('LANET_SERVICE_MODE') == '1'
            auto_register = os.environ.get('LANET_AUTO_REGISTER') == '1'

            if service_mode or auto_register:
                logger.info("Agent not registered, attempting auto-registration in service mode...")

                # Try to get installation token from config
                try:
                    installation_token = config_manager.get('registration.installation_token')
                    if installation_token:
                        logger.info(f"Found installation token, attempting registration: {installation_token[:10]}...")
                        success = agent.register_with_token(installation_token)

                        if success:
                            logger.info("✅ Auto-registration successful!")
                            # Reload configuration after registration
                            config_manager.reload()
                        else:
                            logger.error("❌ Auto-registration failed!")
                            if service_mode:
                                # In service mode, we can't show UI, so exit with error
                                sys.exit(1)
                    else:
                        logger.error("No installation token found in configuration")
                        if service_mode:
                            sys.exit(1)
                except Exception as e:
            ...
            # If still not registered and not in service mode, show installation window
            if not agent.is_registered() and not service_mode:
                ...
                config_manager.reload()
            elif not agent.is_registered() and service_mode:
                logger.error("Agent not registered and running in service mode - cannot continue")
                sys.exit(1)
            agent = AgentCore(config_manager, ui_enabled=not args.no_ui)
