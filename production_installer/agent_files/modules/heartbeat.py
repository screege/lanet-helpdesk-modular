...
class HeartbeatModule:
    ...
    def __init__(self, config_manager, database_manager, monitoring_module=None):
        ...
        self.heartbeat_interval = self.config.get('agent.heartbeat_interval', 900)  # 15 minutes for status
        self.inventory_interval = self.config.get('agent.inventory_interval', 86400)  # 24 hours for full inventory
        self.critical_check_interval = self.config.get('agent.critical_check_interval', 300)  # 5 minutes for critical checks

        self.logger.info(f"Heartbeat module initialized:")
        self.logger.info(f"  - Status heartbeat: {self.heartbeat_interval}s ({self.heartbeat_interval/60:.1f} min)")
        self.logger.info(f"  - Full inventory: {self.inventory_interval}s ({self.inventory_interval/3600:.1f} hours)")
        self.logger.info(f"  - Critical checks: {self.critical_check_interval}s ({self.critical_check_interval/60:.1f} min)")
    ...
    def start(self):
        ...
        while self.running:
            try:
                ...
                # Wait for next heartbeat (with responsive stopping)
                self._sleep_with_stop_check(self.heartbeat_interval)
                
            except Exception as e:
                self.logger.error(f"Error in heartbeat loop: {e}", exc_info=True)
                self._sleep_with_stop_check(60)  # Wait 1 minute before retrying
    ...
    def send_heartbeat(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Store heartbeat success in database (skip to prevent hanging)
                # TODO: Fix database hanging issue
        ...
        try:
            ...
            if 'next_heartbeat' in response_data:
                ...
                if next_interval != self.heartbeat_interval:
                    ...
                    # Skip config save to prevent hanging
                    # TODO: Fix config save hanging issue
    ...
    def _log_heartbeat_history(self, status: str, response_time: Optional[float], error_message: str = None):
        ...
        try:
            ...
            # Store with timestamp key to avoid conflicts (skip to prevent hanging)
            # TODO: Fix database hanging issue
    ...
    def _sleep_with_stop_check(self, seconds):
        ...
        while time.time() < end_time and self.running:
            time.sleep(min(1, end_time - time.time()))  # Check every second
