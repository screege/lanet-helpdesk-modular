...
from modules.heartbeat_simple import SimpleHeartbeatModule
...
class AgentCore:
    ...
    def __init__(self, config_manager: ConfigManager, ui_enabled: bool = True):
        ...
        self.heartbeat = SimpleHeartbeatModule(self.config, self.database)
    ...
    def is_registered(self) -> bool:
        ...
        if asset_id is not None and agent_token is not None:
            # Verify registration is still valid with backend
            try:
                if self.registration.verify_registration():
                    self.logger.info(f"Agent is registered with asset_id: {asset_id}")
                    return True
                else:
                    self.logger.warning("Agent registration is invalid, clearing local data")
                    # Clear invalid registration data
                    self.database.set_config('asset_id', None)
                    self.database.set_config('agent_token', None)
                    return False
            except Exception as e:
                self.logger.error(f"Error verifying registration: {e}")
                # Assume registered if we can't verify (network issues, etc.)
                return True
